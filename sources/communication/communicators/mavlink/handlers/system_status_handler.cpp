...
void SystemStatusHandler::processMessage(const mavlink_message_t& message)
{
    ...
    node->childNode(telemetry::ahrs)->setParameters(
    { { telemetry::present, status.onboard_control_sensors_present & MAV_SYS_STATUS_AHRS },
      { telemetry::enabled, status.onboard_control_sensors_enabled & MAV_SYS_STATUS_AHRS },
      { telemetry::operational, status.onboard_control_sensors_health & MAV_SYS_STATUS_AHRS } });
